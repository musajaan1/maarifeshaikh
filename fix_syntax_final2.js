...
const searchStartIndex = code.indexOf('const articleData = {', 40000); // start searching after the first two occurrences
...
if (search.includes('title: document.getElementById(\'homeTitle\').value')) {
    fs.writeFileSync('admin/admin.js', code.replace(search, replace));
    console.log('Fixed');
} else {
    console.log('Not found correct search block. Search block found:', search);
}
